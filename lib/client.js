window.__ModuleLoader__.load({
		...
		var Model = class {
			constructor() {
				...
				this.app = new PIXI.Application({
					view: document.getElementById("live2d-mygo"),
					autoStart: true,
					width: 800,
					height: 800,
					backgroundAlpha: 0
    ...
    <div id="texture-selection-panel-mygo" class="waifu-panel waifu-panel-mygo" style="display: none;"></div>`);
			...
			for (const tool of getConfig().tools) {
				...
				toolBar.insertAdjacentHTML("beforeend", `<span id="waifu-tool-mygo-${tool}" title="${TOOL_TITLES[tool] || tool}">${decodeURIComponent(icon).replace("data:image/svg+xml,", "")}</span>`);
			...
			hooks.on(document, "click", (event) => {
				if (event.target.closest("#model-selection-panel-mygo") || event.target.closest("#texture-selection-panel-mygo") || event.target.closest("#waifu-tool-mygo") || event.target.closest("#waifu-toggle-mygo")) return;
				closePanels();
			});
		...
		function enableDrag(widgetEl) {
			...
			widgetEl.addEventListener("pointerdown", (event) => {
				if (event.target.closest("#waifu-tool-mygo") || event.target.closest(".waifu-panel-mygo") || event.target.closest("#waifu-toggle-mygo")) return;
				drag.active = true;
				drag.moved = false;
			...
			const endDrag = (event) => {
				...
				if (drag.moved) {
					const rect = widgetEl.getBoundingClientRect();
					try {
						localStorage.setItem("mygo-waifu-pos", JSON.stringify({
							left: rect.left,
							top: rect.top
						}));
					} catch (error) {}
		...
		*/
		async function initWidget(config) {
			...
			if (localStorage.getItem("mygo-waifu-display") && Date.now() - localStorage.getItem("mygo-waifu-display") <= 864e5) {
...
#waifu-mygo, #waifu-toggle-mygo { z-index: 2147483646 !important; }
.waifu-panel { z-index: 2147483647 !important; }
#waifu-mygo { left: auto; right: 460px; top: 460px; bottom: auto; }
`;
